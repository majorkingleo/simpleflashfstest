...
public:
	...
	bool seek( std::size_t pos_ ) override {
	 ...
	 // Truncate the underlying file to `new_size` bytes by closing it,
	 // calling std::filesystem::resize_file, and reopening with the
	 // original openmode (minus any trunc/app flags so we don't wipe or
	 // re-append). Required by SimpleIni's shrink path so the file
	 // doesn't keep growing with stale whitespace padding.
	 bool truncate( std::size_t new_size ) override {
		 file.flush();
		 file.close();

		 std::error_code ec;
		 std::filesystem::resize_file( name, new_size, ec );
		 if( ec ) {
			 // Reopen anyway so the handle isn't left closed.
			 auto reopen_mode = openmode & ~std::ios_base::trunc & ~std::ios_base::app;
			 file.open( name.c_str(), reopen_mode );
			 return false;
		 }
		 ...
		 // Position at end so subsequent tellg/seek behaves sensibly.

...
public:
	...
	void init()
	{
		...
		// Must pass SFF_PAGE_SIZE explicitly; otherwise the 3-arg overload
		// SimSTM32InternalFlashPc(filename, PAGE_SIZE, do_mem_mapping) is
		// selected, which assumes the file already exists and throws a
		// std::filesystem::filesystem_error from SimFlashFsFlashMemory's
		// 1-arg ctor (file_size = std::filesystem::file_size(filename))
		// because clear() just removed it.
	...
	void init_mapped()
	{
		...
		// AI generated by GitHub Copilot Claude Opus 4.7 START
		// See init() above for why SFF_PAGE_SIZE must be passed explicitly.
		mem1.emplace(file1,SFF_PAGE_SIZE,SFF_MAX_SIZE,true);
		mem2.emplace(file2,SFF_PAGE_SIZE,SFF_MAX_SIZE,true);
		// AI generated by GitHub Copilot Claude Opus 4.7 END
		H7TwoFace::set_memory_interface(&mem1.value(),&mem2.value());
	}
